<div class="format">
	<h1>FTC Skit Committee</h1>
	<p>
		Enter stage left! Welcome to the FTC Skit Committee! The FTC Skit is committed to presenting an
		exciting skit filled with jokes, creative backdrops, and dancing!
	</p>
	<div class="flex">
		<div class="lg:hover:scale-110 transition duration-300 ease-in-out">
			<img
				class="frame shadow"
				src="../src/lib/images/leadership/committee/ftcskit/jungsuk_ryow.jpg"
				alt=""
			/>
			<p>FTC Skit Chair - Jungsuk Ryow</p>
		</div>
		<div class="lg:hover:scale-110 transition duration-300 ease-in-out">
			<img
				class="frame shadow"
				src="../src/lib/images/leadership/committee/ftcskit/brandon_tran.jpg"
				alt=""
			/>
			<p>Executive Assistant - Brandon Tran</p>
		</div>
	</div>
</div>

<style>
	.format {
		background-color: var(--bg-color);
		color: var(--textcolor);
		text-align: center;
	}

	h1 {
		font-size: var(--size-xl);
		padding: 2.5rem 0 0.5rem 0;
	}

	p {
		font-size: var(--size-md);
		margin: auto 20%;
		padding: 20px 0 0 0;
	}

	.flex {
		display: flex;
		flex-wrap: wrap;
		justify-content: center;
		object-fit: cover;
		padding: 50px;
		gap: 25px;
	}

	.flex img {
		max-width: 300px;
	}

	.frame {
		border: 10px ridge #cccccc;
		background: #4c4c4c;
	}

	.shadow {
		box-shadow:
			0px 10px 13px -7px #000000,
			10px 12px 11px 4px rgba(0, 0, 0, 0);
	}

	/*Small Tablet*/
	@media screen and (min-width: 620px) {
	}

	/*Large Tablet and Laptop*/
	@media screen and (min-width: 960px) {
	}

	/*Desktop*/
	@media screen and (min-width: 1200px) {
	}
</style>
